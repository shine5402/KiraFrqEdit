...
use kira_frq_core::{frq, mrq};

const BIN: &str = env!("CARGO_BIN_EXE_kira-frqgen");
...
fn kira(dir: &Path, args: &[&str]) -> Run {
    let output: Output = Command::new(BIN)
        .args(args)
        .current_dir(dir)
        .stdin(Stdio::null())
        .output()
        .expect("kira-frqgen runs");
    Run {
...
fn quiet_keeps_failures_only() {
    ...
    let failed = kira(&scratch.0, &["bank", "-q"]);
