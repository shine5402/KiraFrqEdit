...
fn prefer_clang_cl(build: &mut cc::Build) {
    if std::env::var_os("CC").is_some() || std::env::var_os("CXX").is_some() {
        println!("cargo:warning=kira-frq-world: CC/CXX set in the environment; cc honors them");
        return;
    }
    ...
    match find_clang_cl() {
        Some(clang_cl) => {
            println!(
                "cargo:warning=kira-frq-world: using clang-cl at {}",
                clang_cl.display()
            );
            build.compiler(clang_cl);
        }
        None => {
            println!("cargo:warning=kira-frq-world: clang-cl not found; falling back to MSVC cl");
        }
    }
}
