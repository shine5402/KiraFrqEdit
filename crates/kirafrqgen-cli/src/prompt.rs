...
use kirafrqgen_core::{RunPlan, Target};
...
pub fn ask_llsm() -> bool {
    loop {
        eprint!("kirafrqgen-cli: delete cached .llsm files after mrq writes? [Y/n] ");
        ...
        match std::io::stdin().lock().read_line(&mut answer) {
            ...
            Ok(_) => match answer.trim().to_ascii_lowercase().as_str() {
                "" | "y" | "yes" => return true,
                "n" | "no" => return false,
                _ => eprintln!("kirafrqgen-cli: please answer y or n"),
            },
        }
    }
}
...
mod tests {
    ...
    use kirafrqgen_core::{FilePlan, RunPlan};
