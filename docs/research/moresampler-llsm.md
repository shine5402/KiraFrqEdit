...
- `load-frq` was not exercised black-box here (our config had it off); the tutorial documents it:
  `strict` loads `.frq` pitch when `desc.mrq` is unavailable, `on` uses `.frq` to correct
  moresampler's own estimate, and the result is written to `desc.mrq` either way.
